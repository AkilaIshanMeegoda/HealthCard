...
const DoctorDetailsPage = () => {
  ...
  useEffect(() => {
    const fetchDoctorDetails = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/api/doctors/doctor/${doctorId}`);
  ...
  if (!doctor) {
    return <div className="flex justify-center items-center min-h-screen"><p className="text-xl">No doctor details available.</p></div>;
  }
        ...
        <div className="px-6 pb-6">
          ...
          {doctor.availability && doctor.availability.length > 0 ? (
            doctor.availability.map((slot) => (
              <div key={slot._id} className="bg-gray-50 p-4 mb-2 rounded border border-gray-200">
                <p className="text-gray-800 font-semibold">Date: {slot.date || 'Not specified'}</p>
                <p className="text-gray-800">Time: {slot.time || 'Not specified'}</p>
                <p className="text-gray-800">Status: {slot.status || 'Status not available.'}</p>
              </div>
            ))
          ) : (
          ...
          <p className="text-gray-600">{doctor.description || 'No description available.'}</p>
        </div>
      </div>
    </div>
  );
